#include "fmtlog/fmtlog.h"  // IWYU pragma: keep
#include "ph.hpp"

#include "cstdlib"
#include "mutex"
#include "optional"
#include "vector"

namespace PROBES {

using namespace nlohmann;
using namespace std::literals;

Ph::Ph(std::mutex &mutex, MODBUS::Modbus &modbus, json &ph_conf)
    : modbus(modbus), conf(ph_conf), mutex(mutex) {
  name = conf.value("name", "ph");
  addr = conf.value("addr", 1);
  value_reg = conf.value("value_reg", 0);
  loop = conf.value("interval", 5000);
  offset_a = conf.value("offset_a", 1.0f);
  offset_b = conf.value("offset_b", 0.0f);
  value_min = conf.value("value_min", 4.0f);
  value_max = conf.value("value_max", 9.0f);
  random_fact = conf.value("random_fact", 0.5f);
  enable = conf.value("enable", true);

  logi("{} probe, addr: {}", name, addr);

  modbus.set_slave(addr);
  std::srand(std::time(0));
}

void Ph::update_value_kacise() {
  modbus.set_slave(addr);

  auto sleep =
      [](std::chrono::time_point<std::chrono::high_resolution_clock> start,
         int loop) {
        auto end = std::chrono::high_resolution_clock::now();
        auto duration =
            std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
                .count();
        auto delay = ((loop - duration) > 0) ? loop - duration : 0;
        std::this_thread::sleep_for(std::chrono::milliseconds(delay));
      };

  auto start = std::chrono::high_resolution_clock::now();
  if (!enable) {
    logi("{}, reg: {}, disabled", name, addr);
    fmtlog::poll();
    sleep(start, loop);
    return;
  }
  logi("reading {}, addr: {}, reg: {}", name, addr, value_reg);
  auto v = modbus.get_data(value_reg, 6 * 2);
  if (!v.size()) {
    logi("error reading {}, addr: {}, reg: {}", name, addr, value_reg);
    sleep(start, loop);
    return;
  }
  auto val = modbus.dcba_to_float(v,2);
  logi("success reading {}, addr: {}, reg: {}, value: {}", name, addr, value_reg, val);
  {
    const std::lock_guard<std::mutex> lock(mutex);
    value = val;
  }
  sleep(start, loop);
}

void Ph::update_value_boqu() {
  modbus.set_slave(addr);

  auto sleep =
      [](std::chrono::time_point<std::chrono::high_resolution_clock> start,
         int loop) {
        auto end = std::chrono::high_resolution_clock::now();
        auto duration =
            std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
                .count();
        auto delay = ((loop - duration) > 0) ? loop - duration : 0;
        std::this_thread::sleep_for(std::chrono::milliseconds(delay));
      };

  auto start = std::chrono::high_resolution_clock::now();
  if (!enable) {
    logi("{}, reg: {}, disabled", name, addr);
    fmtlog::poll();
    sleep(start, loop);
    return;
  }
  logi("reading {}, reg: {}", name, addr);

  auto v = modbus.get_value_int16_ab(value_reg);

  if (!v.has_value()) {
    logi("error reading {}, reg: {}", name, addr);
    fmtlog::poll();
    sleep(start, loop);
    return;
  }

  logi("success reading {}, reg: {}, value: {}", name, addr, v.value());

  {
    const std::lock_guard<std::mutex> lock(mutex);
    value = static_cast<float>(v.value()) * 1.0;
  }

  fmtlog::poll();
  sleep(start, loop);
}

void Ph::set_enable_read(bool en) {
  const std::lock_guard<std::mutex> lock(mutex);
  enable = en;
}

bool Ph::get_enable_read() { return enable; }

float Ph::get_value_kacise() {
  auto v = value;

  auto random = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);

  if (enable == false) {
    v = value_min + (random * random_fact);
  }

  v = (v + offset_a) * offset_b;

  if (v < value_min) {
    v = value_min + (random * random_fact);
  } else if (v > value_max) {
    v = value_max - (random * random_fact);
  }

  return v;
}

float Ph::get_value_boqu() {
  auto v = value;

  auto random = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
  random *= random_fact;

  if (enable == false) {
    v = value_min + random;
  }

  v = (v + offset_a) * offset_b;

  if (v < value_min) {
    v = value_min + random;
  } else if (v > value_max) {
    v = value_max - random;
  }

  return v;
}

int Ph::set_offset(float a, float b) {
  {
    const std::lock_guard<std::mutex> lock(mutex);
    offset_a = a;
    offset_b = b;
    conf.at("offset_a") = offset_a;
    conf.at("offset_b") = offset_b;
  }
  return 0;
}

void Ph::impl_set_calib_boqu_first_sequence() {
  if (!calib_first_sequence) return;

  modbus.set_slave(addr);
  // Standard Solution
  // address 20
  // 0:Standard Solution
  // 4.00,7.00,10.00;
  // 1:Standard Solution
  // 4.00,6.86,9.18
  modbus.set_data_int_ab(20, 1);
  // Instrument Calibration
  // address 13
  modbus.set_data_int_ab(13, 2);
  calib_first_sequence = false;
}

void Ph::update_calib_slope_boqu() {
  if (calib == false) return;
  impl_set_calib_boqu_first_sequence();
  auto v = std::optional<int>{0};
  v = modbus.get_value_int16_ab(15);

  if (v.has_value()) {
    logi("{}, reg: {}, slope value: {}", name, addr, v.value());
    const std::lock_guard<std::mutex> lock(mutex);
    slope_boqu = v.value();
  }
}

int Ph::get_slope_boqu() { return slope_boqu; }

void Ph::set_calib_boqu(bool en) {
  const std::lock_guard<std::mutex> lock(mutex);
  if (en) {
    calib_first_sequence = true;
    calib = true;
  } else {
    calib_first_sequence = false;
    calib = false;
  }
}

bool Ph::get_calib_boqu() { return calib; }

std::vector<float> Ph::get_offset() { return {offset_a, offset_b}; }
}  // namespace PROBES